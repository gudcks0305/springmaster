...
func secureCopyDirectory(ctx context.Context, sourceDirectory *os.File, sourceRoot, destinationRoot, relativeDirectory, destinationDirectory string, excluded map[string]struct{}, diagnostics *[]Diagnostic, state *copyState, depth int) error {
	...
	for _, entry := range entries {
		...
		case unix.S_IFREG:
			...
			state.entries++
		case unix.S_IFLNK:
			manifestEntry, copied, err := copySecureSymlink(sourceDirectory, name, expected, sourceRoot, destinationRoot, relative, destinationPath, excluded, diagnostics)
			if err != nil {
				return err
			}
			if copied {
...
func copySecureSymlink(parent *os.File, name string, expected unix.Stat_t, sourceRoot, destinationRoot, relative, destination string, excluded map[string]struct{}, diagnostics *[]Diagnostic) (ManifestEntry, bool, error) {
	...
	destinationTarget, err := safeDestinationPath(destinationRoot, targetRelative)
	if err != nil {
		return ManifestEntry{}, false, err
	}
	newTarget, err := filepath.Rel(filepath.Dir(destination), destinationTarget)
	if err != nil {
	...
	if linkErr != nil || afterTarget != target || verifyEntryAt(int(parent.Fd()), name, expected) != nil {
		_ = os.Remove(destination)
		return ManifestEntry{}, false, fmt.Errorf("%w: symlink mutated at %q", ErrSourceChanged, filepath.ToSlash(relative))
	}
	targetDigest := sha256.Sum256([]byte(newTarget))
	return ManifestEntry{Path: filepath.ToSlash(relative), Kind: "symlink", Size: int64(len(newTarget)), SHA256: hex.EncodeToString(targetDigest[:])}, true, nil
}
